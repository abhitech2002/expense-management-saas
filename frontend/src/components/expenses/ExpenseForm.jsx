...
- max-h-[90vh]: Scrollable if content too tall
...
- Better UX (see before submitting)

For PDFs:
- Show file icon instead
- Can't preview in browser easily
- Could use iframe in production

DRAG AND DROP
--------------
Events:
- dragenter: Mouse enters drop zone
- dragover: Mouse moving over drop zone
- dragleave: Mouse leaves drop zone
- drop: File dropped
...
Client-side (UX):
- Check size < 5MB
- Check type is JPEG/PNG/PDF
- Show error immediately

Server-side (Security):
- Multer validates again
- Never trust client

FORM STATE MANAGEMENT
---------------------
Single formData object:
- Easy to submit all at once
- Easy to reset
- Easy to pre-fill for edit

Separate receiptFile state:
- File object can't be in JSON
- Needs FormData for upload
...
- max={getTodayDate()}: Can't select future
...
INTERVIEW TIP:
"I built a modal form with drag-and-drop file upload. The component
handles both create and edit modes through initialData prop, implements
client-side validation for immediate feedback, and provides image preview
before upload. The drag-and-drop uses native HTML5 APIs with visual
feedback. File validation happens before upload to prevent failed requests,
and the form properly handles FormData for multipart/form-data encoding."
*/

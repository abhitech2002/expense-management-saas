...
2. If manager/admin + pending: Show approve/reject buttons
3. Click action → Show notes form
4. Confirm → API call → Refresh list

TWO-STEP CONFIRMATION
---------------------
Don't approve/reject immediately:
- Show form for notes
- User can cancel
...
--------------------
canApprove():
- Admin: Always (if pending)
- Manager: Only if assigned reviewer
- Employee: Never

RECEIPT PREVIEW
---------------
Images: Show inline with click to expand
PDFs: Show icon with "View PDF" button
Opens in new tab (target="_blank")

APPROVAL NOTES
--------------
Approve: Optional notes (good practice to comment)
Reject: Required notes (must explain why)
...
INTERVIEW TIP:
"I built an expense detail modal with context-aware approval actions.
The component renders different UI based on user role and expense status,
implements a two-step confirmation for approvals to prevent accidents,
and handles both image and PDF receipts with appropriate preview options.
The approval workflow includes validation - rejections require notes while
approvals are optional. The modal uses a responsive grid layout and
